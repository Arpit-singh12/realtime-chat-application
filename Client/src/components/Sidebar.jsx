...
return (
    ...
    text-white ${selectedUser ? "max-md:hidden" : ''} relative`}>

        <div className='pb-5'>
            <div className='flex justify-between items-center'>
                    <img src={assets.logo} alt="logo" className='max-w-40' />
                    <div className="relative py-2 group">
                            <img srcSet={assets.menu_icon} alt="Menu" className='max-h-5
                            cursor-pointer' />
            ...
            mt-5'>
                    ...
                    <input type="text" className='big-transparent border-none outline-none
                    ...
                    <div className='flex flex-col'>
                            {userDummyData.map((user, index)=>(
                                    ...
                                    cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                                            <img src={user?.profilePic || assets.avatar_icon} alt="" className='w-[35px] aspext-[1/1] rounded-full' />
                                            ...
                                            {/* Per-message badge at right of message row */}
                                            {index > 2 && (
                                                <span className="absolute top-1/2 -translate-y-1/2 right-4 h-5 w-5 flex justify-center items-center rounded-full bg-violet-500/80 text-xs font-bold">
                                                    {index}
                                                </span>
                                            )}
                                    </div>
